...
class DayView extends CheckedTextView {
    ...
    private Drawable generateCircleDrawable(final int color) {
        if (color == Color.TRANSPARENT) {
            ShapeDrawable drawable = new ShapeDrawable(new OvalShape());
            drawable.getPaint().setColor(color);
            return drawable;
        } else {
            return this.getResources().getDrawable(R.drawable.yuan_blue);
        }
    }
    ...
    private void calculateBounds(int width, int height) {
        ...
        if (width >= height) {
            ...
            circleDrawableRect.set(circleOffset + 10, 10, radius + circleOffset - 10, height - 10);
        } else {
            tempRect.set(0, offset, width, radius + offset);
            circleDrawableRect.set(10, circleOffset + 10, width - 10, radius + circleOffset - 10);
        }
    }
}
